...
ready(function () {
  ...
  if (basketBtn.length > 0) {
    ...
    const basketTemplate = basketSidebar.querySelector(
      `.js-template-basket-product`
    );
    ...
    basketBtn.forEach(function (element) {
      element.addEventListener(
        ...
        function (event) {
          ...
          this.closest(".product-cart").classList.add(
            "product-cart__btn_disable"
          );
          const srcImg = this.closest(".product-cart").querySelector(
              ".product-cart__img img"
            ).src,
            titleProduct = this.closest(".product-cart").querySelector(
              ".product-cart__title"
            ).innerText,
            priceProduct =
              this.closest(".product-cart").querySelector(
                ".price-main"
              ).innerText,
            weightProduct =
              this.closest(".product-cart").getAttribute("data-weight");
          ...
          voidProduct
            .querySelector(".basket-item__weight")
